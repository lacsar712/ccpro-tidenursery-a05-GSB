...
def _to_out(pond: Pond, valid: bool) -> PondOut:
    return PondOut(
        id=pond.id,
...
def list_ponds(
    ...
    cal = calibration_map(db, [p.id for p in ponds])
    return [_to_out(p, cal[p.id][1]) for p in ponds]
...
def create_pond(
    ...
    db.refresh(item)
    cal = calibration_map(db, [item.id])
    return _to_out(item, cal[item.id][1])
